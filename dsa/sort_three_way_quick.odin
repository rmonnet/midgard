// This file contains the three way implementation for the quick  sort algorithm.
//
// Quick sort sorts the xs array by recursively partitioning the array
// into three sub-arrays where elements in the first sub-array are less than
// a selected element, elements in the second sub-array are equal to the selected
// element and elements in the third sub-array are greater than
// a selected element.
// This version of the quick sort algorithm is optimized for
// cases where the array contains multiple duplicate keys.
// It avoid falling in the worst case (O(N^2)) which can happen with the
// standard quick sort algorithm when there are duplicate keys.
//
...
three_way_quick_sort :: proc(xs: []$T, less_than: proc(a, b: T) -> bool) {
	...
	sort :: proc(xs: []T, lo, hi: int, less_than: proc(a, b: T) -> bool) {
		...
		i := lo
		for i <= gt {
			if less_than(xs[i], ref) {
				xs[lt], xs[i] = xs[i], xs[lt]
				lt += 1
				i += 1
			} else if less_than(ref, xs[i]) {
		...
		sort(xs, lo, lt - 1, less_than)
		sort(xs, gt + 1, hi, less_than)
	}
...
@(test)
twqs_test_sort_int :: proc(t: ^testing.T) {
	...
	three_way_quick_sort(xs, proc(a, b: int) -> bool {return a < b})
...
@(test)
twqs_test_sort_float :: proc(t: ^testing.T) {

	xs: [1000]f64
	for i in 0 ..< len(xs) {xs[i] = rand.float64()}
	three_way_quick_sort(xs[:], proc(a, b: f64) -> bool {return a < b})
...
@(test)
twqs_test_sort_string :: proc(t: ^testing.T) {
	...
	three_way_quick_sort(xs, proc(a, b: string) -> bool {return a < b})
...
@(test)
twqs_test_sort_string_reverse :: proc(t: ^testing.T) {

	xs := strings.split("qetaljrcipbmnhgfdlbsok", "")
	defer delete(xs)
	expected := strings.split("tsrqponmllkjihgfedcbba", "")
	defer delete(expected)
	three_way_quick_sort(xs, proc(a, b: string) -> bool {return b < a})
